...
- **Do**:
  ...
  2. **Commit 1 — composition methods + `atmosphere_long_form_composition` filter + `META_THREAD_RECORDS` constant, with `'link-card'` default unchanged:**
     ...
     4. Add private method `build_teaser_thread(): array`:
        - **Hook precedence:** if `! empty( $this->object->post_excerpt )`, pass the excerpt through `render_post_content_plain()` (same helper used for body, so whatever inline HTML shortcodes/formatting logic the excerpt carries gets normalized), then clamp to 300 graphemes as a safety floor in case the excerpt exceeds. Use that as the hook. Otherwise fall through to the body path: `truncate_to_budget( $this->render_post_content_plain( $this->object ), 280, $prefer_sentence = true )`. The body hook is the final prose cut before the CTA, so sentence boundary is required. 280 leaves room for future trailing content without blowing the 300 cap.
     ...
     5. Add public method `build_long_form_records(): array` — this is what Publisher calls for long-form posts:
        ...
        - **Empty-body degradation guard (applies only to `'teaser-thread'` and `'truncate-link'`):** compute `$plain = $this->render_post_content_plain( $this->object );` and `$excerpt = $this->object->post_excerpt ?? '';`. If `$strategy` is `'teaser-thread'` or `'truncate-link'` AND both `$plain` and `$excerpt` are below 10 graphemes (rendered post has no meaningful prose to build a hook from), override `$strategy = 'link-card'` and call `trigger_error( … , E_USER_NOTICE )` or `error_log` so ops can tell the fallback from an intentional `'link-card'` configuration. This keeps the worst-case output at today's link-card quality rather than an empty thread post.
        - `switch ( $strategy )`:
        ...
        - Before returning each entry, apply `$record = apply_filters( 'atmosphere_transform_bsky_post', $record, $this->object );` so the existing post-transform hook runs consistently for every record. **Note:** for thread strategies this filter now fires N times per WP post (once per thread record) instead of once. Behavior change for downstream consumers who assumed single-post semantics; documented in changelog. No detection/elision — consumers can branch on record shape if they need to skip reply posts.
  ...
  3. **Commit 2 — tests for composition methods:**
     1. In `tests/phpunit/tests/transformer/class-test-post.php`, add:
        ...
        - `test_build_long_form_records_teaser_thread_uses_excerpt_when_set()` — `'teaser-thread'` with `$post->post_excerpt = "Custom-curated hook copy."`; assert the first thread record's `text` is the excerpt (not a body prefix).
        - `test_build_long_form_records_degrades_to_link_card_when_body_and_excerpt_empty()` — `'teaser-thread'` filter, but `$post->post_content` and `$post->post_excerpt` both empty (or below the 10-grapheme threshold); assert the returned array is 1 entry with `embed` populated (link-card shape), not a thread. Assert a notice / log entry is emitted that distinguishes the fallback from an intentional `'link-card'` configuration.
