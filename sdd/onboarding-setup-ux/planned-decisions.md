...
- **Decision**: On plugin activation, `register_activation_hook` sets a 30-second `fosse_activation_redirect` transient. On the next qualifying `admin_init`, Menu checks for the transient, deletes it, and redirects to the wizard page. The wizard is registered as a hidden admin page (via a null `parent_slug` on `add_submenu_page()`) so it has a real admin URL but never appears in the sidebar.
- **Reason**: This is the standard WordPress onboarding pattern (WooCommerce, Jetpack, Akismet all do it). The transient is consumed at most once within its 30-second TTL — if no admin loads `wp-admin` in that window the redirect doesn't fire at all. The redirect is gated on `current_user_can('manage_options')` and skipped during AJAX/cron/CLI/`activate-multi`, so a lower-privileged or non-admin request can't burn the transient before the actual administrator arrives. The hidden-page registration means the wizard inherits WP capability checks without cluttering the menu.
