...
2. **Deactivation restores native admin surfaces when the standalone backends are active.** When FOSSE stops running, its menus and menu-suppression hooks disappear. If standalone ActivityPub and/or Atmosphere are active as normal WordPress plugins, their native menus should naturally reappear with their last configured options.
...
7. **Standalone-after-FOSSE avoids fatals and gives admins a clear notice.** If a user installs standalone ActivityPub or Atmosphere after FOSSE, FOSSE must not load a bundled copy that would collide with the standalone code. If the standalone plugin file is present but inactive, FOSSE should show an actionable admin notice explaining that the backend is installed but inactive, so FOSSE cannot use its bundled copy.
