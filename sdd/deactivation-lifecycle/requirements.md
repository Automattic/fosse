...
7. **Standalone-after-FOSSE avoids fatals.** If a user installs and *activates* standalone ActivityPub or Atmosphere after FOSSE, FOSSE must not load a bundled copy that would collide with the standalone code. Note: the load model changed in [#228](https://github.com/Automattic/fosse/pull/228) — FOSSE now gates the bundled load on standalone *activation* (constant defined or plugin active), not mere on-disk presence, so an installed-but-inactive standalone no longer suppresses the bundle (federation keeps working); the trade-off is a narrow same-request redeclare-fatal window if the user activates the standalone while the bundle is already loaded. FOSSE still shows no admin notice about an inactive standalone — inactive plugins do nothing, so warning about them solves no real problem.
...
- `sdd/bundled-backends/implementation-notes.md` - original decision to skip the bundled load when standalone plugin files are present on disk. Superseded by [#228](https://github.com/Automattic/fosse/pull/228): the gate is now standalone *activation* (constant/active-plugin), not disk presence, so an inactive standalone no longer disables federation.
